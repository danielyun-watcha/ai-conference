 ...
 * Also returns true if deadlines are TBD (not yet announced).
 */
export function hasUpcomingDeadlines(conference: Conference): boolean {
  if (getNextUpcomingDeadline(conference) !== null) return true;

  // Check if any deadline is TBD
  const allDeadlines = getAllDeadlines(conference);
  if (allDeadlines.some(d => d.date === 'TBD')) return true;

  // Check if conference end date is in the future (event hasn't happened yet)
  if (conference.end) {
    const endDate = new Date(conference.end);
    if (isValid(endDate) && !isPast(endDate)) return true;
  }

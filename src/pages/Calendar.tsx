...
const categoryColors: Record<string, string> = {
  ...
  "recommender-systems": "bg-rose-500",
  "data-mining": "bg-pink-500",
  "information-retrieval": "bg-indigo-500",
  "human-computer-interaction": "bg-emerald-500",
  "web-search": "bg-sky-500",
  "large-language-models": "bg-violet-500",
  "robotics": "bg-green-500",
  "speech": "bg-teal-500",
  "signal-processing": "bg-cyan-500",
  "reinforcement-learning": "bg-yellow-500",
  "computer-graphics": "bg-amber-500",
  "other": "bg-gray-500"
};
...
const knownTags = new Set(Object.keys(categoryColors));
...
// Get all deadline dates for a conference (using new deadlines system)
function getDeadlineDates(conf: Conference): Date[] {
  const deadlines = getAllDeadlines(conf);
  const dates: Date[] = [];
  for (const dl of deadlines) {
    if (dl.date === "TBD") continue;
    try {
...
// Get earliest deadline date for a conference
function getEarliestDeadline(conf: Conference): Date | null {
  const dates = getDeadlineDates(conf);
...
const CalendarPage = () => {
  ...
  const safeParseISO = (dateString: string | undefined | number): Date | null => {
    ...
    try {
      if (typeof dateString === 'object') return null;
  ...
  const getEvents = (date: Date) => {
    return conferencesData.filter((conf: Conference) => {
      const mappedTags = conf.tags?.map(mapTag) || [];
      ...
      const deadlineDates = getDeadlineDates(conf);
      const startDate = safeParseISO(conf.start);
      const endDate = safeParseISO(conf.end);

      const isInCurrentYear = (d: Date | null) => d && d.getFullYear() === currentYear;

      if (showDeadlines && selectedCategories.size === 0) {
        return deadlineDates.some(d => isInCurrentYear(d)) && matchesSearch;
      }

      if (!matchesSearch || (!matchesCategory && selectedCategories.size > 0)) return false;

      const deadlineInYear = showDeadlines && deadlineDates.some(d => isInCurrentYear(d));
  ...
  const getDayEvents = (date: Date) => {
    const deadlines = showDeadlines ? conferencesData.filter(conf => {
      ...
      const matchesCategory = selectedCategories.size === 0 ? true :
        (Array.isArray(conf.tags) && conf.tags.some(tag => selectedCategories.has(mapTag(tag))));
      return deadlineDates.some(d =>
        isSameDay(d, date) && d.getFullYear() === currentYear
      ) && matchesCategory;
    }) : [];
    ...
    const conferences = selectedCategories.size > 0 ? conferencesData.filter(conf => {
      ...
      const matchesCategory = Array.isArray(conf.tags) &&
        conf.tags.some(tag => selectedCategories.has(mapTag(tag)));
  ...
  const getConferenceLineStyle = (date: Date) => {
    ...
    return conferencesData
      ...
      .map(conf => {
        ...
        const mappedTag = conf.tags?.[0] ? mapTag(conf.tags[0]) : "other";
        const color = categoryColors[mappedTag] || "bg-gray-500";
        ...
        <div className="space-y-2 mt-3">
          {deadlines.length > 0 && (
            <div className="flex items-start gap-2">
              <span className="font-medium text-sm text-neutral-900">Deadlines:</span>
              <div className="text-sm text-neutral-900 space-y-1">
                {deadlines.map((dl, i) => {
                  const d = safeParseISO(dl.date);
                  return (
                    <div key={`${dl.type}-${i}`}>
                      {dl.label}: {d ? format(d, 'MMMM d, yyyy') : dl.date}
                      {dl.timezone && <span className="text-neutral-500 text-xs ml-1">({dl.timezone})</span>}
                    </div>
                  );
                })}
              </div>
          ...
          {(conf.city || conf.country) && (
            <div className="flex items-start gap-2">
              ...
              <span className="text-sm text-neutral-900">
                {[conf.venue || conf.city, conf.country].filter(Boolean).join(", ")}
              </span>
            </div>
          )}
  ...
  const categories = orderedCategories
    .filter(category =>
      conferencesData.some(conf =>
        conf.tags?.some(t => mapTag(t) === category)
      )
    )
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="space-y-4">
              {getEvents(new Date()).map((conf: Conference) => {
                const earliestDeadline = getEarliestDeadline(conf);
                const startDate = safeParseISO(conf.start);
                return (
                  <div
                    ...
                    onClick={() => {
                      const targetDate = earliestDeadline || startDate;
                      if (targetDate) {
                        setSelectedDate(targetDate);
                        setSelectedDayEvents({
                          date: targetDate,
                          events: getDayEvents(targetDate)
                        });
                      }
                    }}
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="font-semibold">{conf.title} {conf.year}</h3>
                        {conf.full_name && (
                      ...
                      {earliestDeadline && (
                        <span className="text-sm text-red-500">
                          Deadline: {format(earliestDeadline, 'MMM d, yyyy')}
                        </span>
                      )}
                    </div>
